...
export const alt = "Leg Strength — why strong legs matter more as you age | Bedrock.fit";
...
export default async function Image() {
  const [bold, extrabold] = await Promise.all([archivo(700), archivo(800)]);
  const fonts = [
  ...
  const family = fonts && fonts.length ? "Archivo" : "sans-serif";
    ...
    (
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column" }}>
          <div style={{ display: "flex", fontSize: 108, fontWeight: 800, color: "#F2F4EF", letterSpacing: "-0.03em", lineHeight: 1.02 }}>
            Leg&nbsp;<span style={{ color: "#7FD05A" }}>Strength</span>
          </div>
          <div style={{ display: "flex", marginTop: 22, fontSize: 31, fontWeight: 700, color: "rgba(242,244,239,.62)", lineHeight: 1.35, maxWidth: 1010 }}>
            Why it gets more important, not less, with every decade you add.
          </div>
        </div>

        {/* stat strip — the three numbers the article turns on */}
