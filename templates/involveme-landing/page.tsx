...
const cardStyle: React.CSSProperties = { background: "var(--paper)", border: "1px solid var(--line)", borderRadius: 18, padding: "26px 28px", boxShadow: "0 1px 2px rgba(39,44,39,.04)" };
...
function AdSlot({ label, height }: { label: string; height: number }) {
  return (
    <div style={{ margin: "16px 0" }}>
      <div style={{ font: `700 9px ${mono}`, letterSpacing: ".16em", color: "var(--muted)", margin: "0 0 8px" }}>ADVERTISEMENT</div>
      <div
        style={{
          minHeight: height,
...
const STEPS = [
  { n: "01", title: "Enter", desc: "Your sex, bodyweight, and one lift you do often — weight × reps. That's all we need." },
  { n: "02", title: "Estimate", desc: "We compute your true 1-rep max with the Epley formula, plus rep predictions for every plate load." },
  { n: "03", title: "Benchmark", desc: "Your strength-to-bodyweight ratio is scored against blended global strength standards, from Beginner to Elite." },
  { n: "04", title: "Reveal", desc: "Get your training zones, projected feats, and the athlete archetype you're built to become." },
];
...
const FAQS = [
  {
    q: "What is Bedrock.fit?",
    a: "Bedrock.fit is a free athletic potential calculator. Enter your sex, bodyweight, and one lift (weight × reps) and it estimates your 1-rep max, strength level versus blended global standards, training zones, rep predictions, and a playful athlete archetype.",
  },
  {
    q: "How accurate is the 1-rep max estimate?",
    a: "We use the Epley formula, one of the most widely used 1RM estimators. It's most accurate for sets of 1–10 reps. Like all formulas it's an estimate — treat it as a well-informed starting point, not a guarantee.",
  },
  {
    q: "Where do the strength standards come from?",
    a: "Levels are based on a blend of commonly published strength standards, expressed as multiples of bodyweight for each exercise, adjusted for sex. See the Methodology page for details.",
  },
  {
    q: "Is it really free?",
    a: "Yes. No account, no credit card, no email. Everything runs directly in your browser — nothing you type is sent to a server.",
  },
  {
    q: "Are the projected feats real predictions?",
    a: "They're playful projections derived from your strength profile — meant for fun and motivation, not measured results. Go test them in real life!",
  },
];
...
export default function Home() {
  ...
  return (
    <div style={{ ...rootVars, background: "var(--paper)", minHeight: "100vh", color: "var(--body)", fontFamily: space }}>
      ...
      <header className="lp-nav" style={{ position: "sticky", top: 0, zIndex: 30, background: "rgba(255,255,255,.9)", backdropFilter: "blur(10px)", borderBottom: "1px solid var(--line)" }}>
        <div style={{ ...wrap, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 18, padding: "14px 20px" }}>
          ...
          <nav className="lp-nav-links" aria-label="Main">
            {[["#how-it-works", "How it works"], ["#features", "Features"], ["#archetypes", "Archetypes"], ["#faq", "FAQ"], ["/methodology", "Methodology"]].map(([href, label]) =>
              href.startsWith("/") ? (
                <Link key={href} href={href} style={{ font: `600 14px ${space}`, color: "var(--body)", textDecoration: "none" }}>{label}</Link>
              ) : (
                <a key={href} href={href} style={{ font: `600 14px ${space}`, color: "var(--body)", textDecoration: "none" }}>{label}</a>
              ),
            )}
          </nav>
          <a href="#calculator" style={{ ...btnGreen, padding: "10px 18px", font: `800 13px ${archivo}` }}>Get my strength scan</a>
        </div>
      </header>

      {/* ── hero ── */}
      <section style={{ background: "radial-gradient(1100px 500px at 50% -80px, var(--mint2), var(--paper) 75%)", padding: "72px 0 40px", textAlign: "center" }}>
        <div style={wrap}>
          ...
          <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap", marginTop: 28 }}>
            <a href="#calculator" style={btnGreen}>Reveal my potential →</a>
            <a href="#how-it-works" style={{ ...btnPrimary, background: "#fff", color: "var(--ink)", border: "1px solid var(--line)" }}>See how it works</a>
          </div>
      ...
      {/* ── calculator ── */}
      <section id="calculator" style={{ padding: "28px 0 40px", scrollMarginTop: 80 }}>
        <div style={wrap}>
          <div style={{ textAlign: "center", marginBottom: 26 }}>
            <h2 style={h2Style}>Run your free strength scan</h2>
            <p style={{ font: `500 15px/1.6 ${space}`, color: "var(--muted)", maxWidth: 540, margin: "10px auto 0" }}>
              A few numbers and one lift. Results appear instantly — nothing is uploaded or stored.
            </p>
          </div>

          <div style={{ ...cardStyle, background: "var(--mint)", padding: "24px 28px" }}>
            <div className="bf-hero">
              ...
              <div>
                ...
                {res ? (
                  <div style={{ background: "#fff", border: "2px solid var(--green)", borderRadius: 14, padding: "24px 20px", textAlign: "center" }}>
                  ...
                  <div style={{ background: "#fff", border: "1px dashed var(--line)", borderRadius: 14, padding: "44px 20px", textAlign: "center", color: "var(--muted)", font: `600 13px ${space}` }}>
            ...
            <>
              <div className="bf-two" style={{ marginTop: 16 }}>
                <div style={cardStyle}>
                  ...
                  <div className="bf-two-tight">
                    <div style={statBox}>
                      <div style={{ font: `900 30px ${archivo}`, color: "var(--green-dark)" }}>{res.oneRM}</div>
                ...
                <div style={cardStyle}>
                  ...
                  {[["Max strength", "heavy, low reps", res.zStr], ["Muscle size", "moderate, medium reps", res.zHyp], ["Endurance", "light, high reps", res.zEnd]].map(([t, s, v]) => (
                    <div key={t} style={{ display: "flex", justifyContent: "space-between", padding: "12px 14px", borderRadius: 10, background: "var(--mint)", border: "1px solid var(--line)", marginTop: 8 }}>
                      <div>
              ...
              <div style={{ ...cardStyle, marginTop: 16 }}>
                ...
                <div className="bf-three">
                  <div style={statBox}>
                    <div style={{ font: `900 30px ${archivo}`, color: "var(--green-dark)" }}>{res.pullups}</div>
                    <div style={{ font: `600 12px ${space}`, color: "var(--muted)" }}>Max pull-ups</div>
                  </div>
                  <div style={statBox}>
                    <div style={{ font: `900 30px ${archivo}`, color: "var(--green-dark)" }}>{res.fivek}</div>
                    <div style={{ font: `600 12px ${space}`, color: "var(--muted)" }}>5K run time</div>
                  </div>
                  <div style={statBox}>
                    <div style={{ font: `900 20px ${archivo}`, color: "var(--green-dark)", marginTop: 6 }}>{res.muscleup}</div>
      ...
      {/* ── how it works ── */}
      <section id="how-it-works" style={{ background: "var(--mint)", padding: "64px 0", scrollMarginTop: 70 }}>
        <div style={wrap}>
          <div style={{ textAlign: "center", maxWidth: 680, margin: "0 auto 40px" }}>
            <h2 style={h2Style}>From one set in the gym to a full strength profile</h2>
            <p style={{ font: `500 15px/1.65 ${space}`, color: "var(--muted)", margin: "12px 0 0" }}>
              You stay in the driver&rsquo;s seat — we just do the math. Here&rsquo;s the whole journey.
            </p>
          </div>
          <div className="lp-steps">
            {STEPS.map((s) => (
              <div key={s.n} style={{ background: "#fff", border: "1px solid var(--line)", borderRadius: 16, padding: "24px 22px" }}>
                <div style={{ font: `900 13px ${archivo}`, color: "var(--green)", letterSpacing: ".06em" }}>{s.n}</div>
                <h3 style={{ font: `800 19px ${archivo}`, color: "var(--ink)", margin: "8px 0 8px" }}>{s.title}</h3>
                <p style={{ font: `500 13.5px/1.6 ${space}`, color: "var(--muted)", margin: 0 }}>{s.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── stats band ── */}
      <section style={{ background: "var(--charcoal)", padding: "56px 0" }}>
        <div style={{ ...wrap, textAlign: "center" }}>
          <div className="lp-stats">
            {[["1 lift", "is all it takes"], ["~30 sec", "to your full profile"], ["0 signups", "free, no account, no data stored"]].map(([big, small]) => (
              <div key={big}>
                <div style={{ font: `900 clamp(34px, 4vw, 46px) ${archivo}`, color: "#7ede9f" }}>{big}</div>
                <div style={{ font: `600 14px ${space}`, color: "rgba(255,255,255,.75)", marginTop: 4 }}>{small}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="archetypes" style={{ background: "var(--mint)", padding: "64px 0", scrollMarginTop: 70 }}>
        <div style={wrap}>
          ...
          <div style={{ textAlign: "center", marginTop: 34 }}>
            <a href="#calculator" style={btnGreen}>Find my archetype →</a>
          </div>
        </div>
      </section>

      {/* ── FAQ ── */}
      <section id="faq" style={{ padding: "64px 0", scrollMarginTop: 70 }}>
        <div style={{ ...wrap, maxWidth: 760 }}>
          <h2 style={{ ...h2Style, textAlign: "center", marginBottom: 34 }}>FAQs about Bedrock.fit</h2>
      ...
      {/* ── final CTA band ── */}
      <section style={{ background: "linear-gradient(120deg, var(--green-dark), var(--green))", padding: "64px 0", textAlign: "center" }}>
        <div style={wrap}>
          <h2 style={{ font: `800 clamp(28px, 4vw, 40px)/1.15 ${archivo}`, color: "#fff", margin: 0 }}>Try Bedrock.fit now</h2>
          <p style={{ font: `500 16px/1.6 ${space}`, color: "rgba(255,255,255,.85)", margin: "12px auto 0", maxWidth: 480 }}>
            Start unlocking your athletic potential today. One lift, thirty seconds, zero cost.
          </p>
          <a href="#calculator" style={{ ...btnPrimary, background: "#fff", color: "var(--green-dark)", marginTop: 26 }}>Get started for free</a>
          <p style={{ font: `500 12px ${space}`, color: "rgba(255,255,255,.7)", marginTop: 10 }}>No credit card required</p>
        </div>
      </section>
        ...
        <div style={wrap}>
          <div className="lp-footer-cols">
            ...
            <div>
              ...
              <a href="#calculator" className="lp-flink">Strength calculator</a>
              <a href="#how-it-works" className="lp-flink">How it works</a>
      ...
      {/* Avatar Assistant */}
      <AvatarWindow autoOpen={true} demoText="Hey! I'm your Bedrock strength assistant. Enter your stats and one lift you do, and I'll reveal your true potential. Let's go!" />
    </div>
  );
}
